...
const userController = {
        ...
        try {
            ...
            async function continueRegister() {
                try {
                    ...
                    res.cookie('verificationToken', verificationToken, { httpOnly: true, secure: false, sameSite: 'None', maxAge: 1000 * 60 * 30 * 30}); 
    ...
    getExperienceAllUsers: async (req, res) => {
        try {
            ...
            const users = await User.find().select('username email experience role -_id');
            if (!users || users.length === 0) {
                throw new NotFound({ message: 'No users found', req }, 'info');
            }
    ...
    resendEmail: async (req, res) => {
        try {
            ...
            await user.save();
            res.cookie('verificationToken', verificationToken, { httpOnly: true, secure: false, sameSite: 'None', maxAge: 1000 * 60 * 60 * 2 });
